...
class EmojiManager {
  ...
  var emojisForCategory: [EmojiCategorys:[Emoji]] = [:]
    ...
    do {
      ...
      if let jsonArray = try JSONSerialization.jsonObject(with: emojisListData!, options: .allowFragments) as? [[String: Any]] {
        ...
        jsonArray.forEach { (emoji: [String:Any]) in
          ...
          var category: EmojiCategorys? = nil
          if let categoryName = emoji["category"] as? String {
            category = EmojiCategorys(rawValue: categoryName)
          }
  ...
  func getEmojisForCategory(_ category: EmojiCategorys) -> [Emoji]? {
