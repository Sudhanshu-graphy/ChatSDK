...
@IBDesignable open class AttributedLabel: UIView {
    ...
    open override func prepareForInterfaceBuilder() {
        ...
        let gray = Style("gray").foregroundColor(UIColor.gray)
    ...
    open override func layoutSubviews() {
        ...
        if let attributedText = state.attributedText {
            ...
            highlightableDetections.forEach { detection in
                ...
                case .tag(_), .hashtag(_),
                     .regex(_), .phoneNumber(_),
                     .link(_), .textCheckingType(_, _),
                     .range:
                    let nsrange = NSRange(detection.range, in: text)
                    textView.layoutManager.enumerateEnclosingRects(forGlyphRange: nsrange, withinSelectedGlyphRange: NSRange(location: NSNotFound, length: 0), in: textView.textContainer, using: { (rect, _) in
                        self.addDetectionAreaButton(frame: rect, detection: detection, text: String(attributedText.string[detection.range]))
                    })
            
                case .mention(let mentionString):
                    text = text.replacingOccurrences(of: "<", with: "")
                    text = text.replacingOccurrences(of: ">", with: "")
                    let mentionText = mentionString.replacingOccurrences(of: ">", with: "")
                    if let nsRange = text.range(of: mentionText)?.nsRange(in: text) {
                        textView.layoutManager.enumerateEnclosingRects(forGlyphRange: nsRange, withinSelectedGlyphRange: NSRange(location: NSNotFound, length: 0), in: textView.textContainer, using: { (rect, _) in
                            self.addDetectionAreaButton(frame: rect, detection: detection, text: String(attributedText.string[detection.range]))
                        })
                    }
...
//        button.layer.borderWidth = 1
    ...
    private func updateAttributedTextInTextView(_ string: NSAttributedString) {
        ...
        string.enumerateAttributes(in: NSRange(location: 0, length: length), options: .longestEffectiveRangeNotRequired, using: { (attributes, range, _) in
