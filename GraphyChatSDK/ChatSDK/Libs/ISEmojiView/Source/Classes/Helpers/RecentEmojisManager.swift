...
final internal class RecentEmojisManager {
    ...
    internal func add(emoji: Emojis, selectedEmoji: String) -> Bool {
    ...
    internal func recentEmojis() -> [Emojis] {
        guard let data = UserDefaults.standard.data(forKey: RecentEmojisKey) else {
            return []
        }
        
        guard let emojis = try? JSONDecoder().decode([Emojis].self, from: data) else {
            return []
        }
